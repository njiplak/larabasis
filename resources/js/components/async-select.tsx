...
> = TMode extends 'multiple' ? TValue[] : TValue;

type AsyncSelectOnChange<
    T,
    TValue,
    TMode extends 'single' | 'multiple',
...
type AsyncSelectInitialOptions<
    T,
    TMode extends 'single' | 'multiple',
> = TMode extends 'multiple' ? T[] : T | undefined;
...
>({
        ...
        if (initialOptions) {
            ...
            if (optionsArray.length > 0) {
                ...
                setOptions((prev) => {
                    ...
                    const newOptions = optionsArray.filter(
                        (opt) => !existingValues.includes(getOptionValue(opt)),
                    );
            ...
            try {
                ...
                for (const valueToFetch of valuesToFetch) {
                    try {
                        ...
                        console.error(
                            `Failed to fetch value ${valueToFetch}:`,
                            err,
                        );
                    }
                }
                ...
                if (allResults.length > 0 && mountedRef.current) {
                    setSelectedOptionsCache((prev) => {
                        const existingValues = prev.map((opt) =>
                            String(getOptionValue(opt)),
                        );
                        const newOptions = allResults.filter(
                            (opt) =>
    ...
    React.useEffect(() => {
        if (currentValues.length > 0 && fetchSelectedValue) {
            const missingValues = currentValues.filter((val) => {
                const existsInCache = selectedOptionsCache.some(
    ...
    const handleSelect = (selectedValue: string) => {
        ...
        if (mode === 'multiple') {
            ...
            const actualCurrentValues =
                (value as TValue[]) || (defaultValue as TValue[]) || [];
            // Check if already selected - need to compare properly
            const isAlreadySelected = actualCurrentValues.some(
                (val) => String(val) === selectedValue,
            );

            if (isAlreadySelected) {
                // Remove from selection
                const newValues = actualCurrentValues.filter(
                    (val) => String(val) !== selectedValue,
                );
                const newOptions = selectedOptionsCache.filter(
                ...
                if (
                    maxSelections &&
                    actualCurrentValues.length >= maxSelections
                ) {
                    return;
                }
                // Add to selection
            ...
            (onChange as AsyncSelectOnChange<T, TValue, 'single'>)(
                optionValue,
                selectedOption,
            );
            setOpen(false);
        ...
        (valueToRemove: TValue) => {
            if (mode === 'multiple') {
                ...
                const actualCurrentValues =
                    (value as TValue[]) || (defaultValue as TValue[]) || [];
                const newValues = actualCurrentValues.filter(
                    (val) => String(val) !== String(valueToRemove),
                );
                const newOptions = selectedOptionsCache.filter(
                    (opt) =>
                        String(getOptionValue(opt)) !== String(valueToRemove),
                );
                setSelectedOptionsCache(newOptions);
                (onChange as AsyncSelectOnChange<T, TValue, 'multiple'>)(
                    newValues,
        ...
        return currentValues
            .map((val) => {
                ...
                const optionInList = options.find(
                    (option) => String(getOptionValue(option)) === String(val),
                );
                if (optionInList) return optionInList;

                // Then try cached selected options
                const cachedOption = selectedOptionsCache.find(
                    (option) => String(getOptionValue(option)) === String(val),
                );
            ...
            return (
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-muted-foreground">
                        +{selectedCount - maxSelectedDisplay} more
                    </span>
                </div>
            );
        }
            ...
            <Popover open={open} onOpenChange={handleOpenChange}>
                <PopoverTrigger asChild>
                    <Button
                        ...
                        disabled={disabled}
                        className={cn(
                            'min-h-10 w-full justify-between bg-background text-left font-normal',
                                    ...
                                    {maxResults &&
                                        options.length >= maxResults && (
                                            <div className="border-t px-2 py-1.5 text-xs text-muted-foreground">
                                                ...
                                                results. Refine your search for
